...
execute_tpchquery_spark() {
  ...
  if [[ "$NATIVE_FORMAT" == "text" ]]; then
    if [[ "$NATIVE_API" == "DS" ]]; then
      ...
      execute_spark "tpch_query_$query" "--class main.scala.TpchQuery $NATIVE_SPARK_LOCAL_DIR/spark-tpc-h-queries_2.11-1.0-final-txt.jar $SCALE_FACTOR $BENCH_CURRENT_NUM_RUN $query $NATIVE_INPUT_DIR $NATIVE_OUTPUT_DIR $NATIVE_OUT_FORMAT $NATIVE_SPRINT $NATIVE_FORMAT $NATIVE_API" "time"    
      #execute_spark "tpch_query_$query" "--class main.scala.TpchQuery $NATIVE_SPARK_LOCAL_DIR/spark-tpc-h-queries_2.11-1.0-D2F-null.jar $SCALE_FACTOR $BENCH_CURRENT_NUM_RUN $query" "time"
      #execute_spark "tpch_query_$query" "--class main.scala.TpchQuery $NATIVE_SPARK_LOCAL_DIR/spark-tpc-h-queries_2.11-1.0-D2F-NOVAR.jar $query $SCALE_FACTOR" "time"
    ...
    if [[ "$NATIVE_API" == "DS" ]]; then
      ...
      execute_spark "tpch_query_$query" "--class main.scala.TpchQuery $NATIVE_SPARK_LOCAL_DIR/spark-tpc-h-queries_2.11-1.0-final.jar $SCALE_FACTOR $BENCH_CURRENT_NUM_RUN $query $NATIVE_INPUT_DIR $NATIVE_OUTPUT_DIR $NATIVE_OUT_FORMAT $NATIVE_SPRINT $NATIVE_FORMAT $NATIVE_API" "time"    
      #execute_spark "tpch_query_$query" "--class main.scala.TpchQuery $NATIVE_SPARK_LOCA
    elif [[ "$NATIVE_API" == "DF" ]]; then
