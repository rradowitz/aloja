...
benchmark_suite_run() {
  ...
  execute_hadoop_new "$bench_name" "fs -mkdir -p /user/eventLog"
...
execute_tpchquery_spark() {
  ...
  if [[ "$NATIVE_FORMAT" == "text" ]]; then
    execute_spark "tpch_query_$query" "--class main.scala.TpchQuery $NATIVE_SPARK_LOCAL_DIR/spark-tpc-h-queries_2.11-1.0-txt.jar
                   $BENCH_CURRENT_NUM_RUN $query $NATIVE_INPUT_DIR $NATIVE_OUTPUT_DIR $NATIVE_FORMAT $NATIVE_OUT_FORMAT
                   $NATIVE_SPRINT $NATIVE_FILTER" "time"
  elif [[ "$NATIVE_FORMAT" == "orc" || "$NATIVE_FORMAT" == "parquet" || "$NATIVE_FORMAT" == "json" ]]; then
    execute_spark "tpch_query_$query" "--class main.scala.TpchQuery $NATIVE_SPARK_LOCAL_DIR/spark-tpc-h-queries_2.11-1.0-orc.jar
                   $BENCH_CURRENT_NUM_RUN $query $NATIVE_INPUT_DIR $NATIVE_OUTPUT_DIR $NATIVE_FORMAT $NATIVE_OUT_FORMAT 
                   $NATIVE_SPRINT $NATIVE_FILTER" "time"
  elif [[ "$NATIVE_FORMAT" == "tbl" ]]; then
  execute_spark "tpch_query_$query" "--class main.scala.TpchQuery $NATIVE_SPARK_LOCAL_DIR/spark-tpc-h-queries_2.11-1.0-tbl.jar
                 $BENCH_CURRENT_NUM_RUN $query $NATIVE_OUTPUT_DIR $NATIVE_OUT_FORMAT $NATIVE_SPRINT $NATIVE_FILTER $NATIVE_DB" 
                 "time"
  else
